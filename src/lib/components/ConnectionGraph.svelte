<script>
  ...
  function initializeGraph() {
    ...
    const strongGradient = defs.append('linearGradient')
      .attr('id', 'strong-connection')
      .attr('gradientUnits', 'userSpaceOnUse');
    
    strongGradient.append('stop')
      .attr('offset', '0%')
  ...
  function updateGraph() {
    ...
    // Create simulation
    simulation = d3.forceSimulation(connections.nodes)
      .force('link', d3.forceLink(connections.links).id(d => d.id).distance(d => {
    ...
    const link = g.append('g')
      ...
      .attr('stroke-dasharray', d => d.strength === 'weak' ? '5,5' : 'none');
